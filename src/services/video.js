...
export function LookForClassify() {
  return axios.get('http://api.hiifire.com/v1/kol-category')
}

// 网红平台列表
export function LookForPlatform() {
  return axios.get('http://api.hiifire.com/v1/plat')
}

// 全部素材（文章）

export function AllArticle() {
  // return axios.get('http://api.hiifire.com/v1/article')
...
export function ArticleDetails(id) {
  return axios({
    url: 'http://api.hiifire.com/v1/article/view',
    params: {
      id
    }
  })
...
export function VideoClassify() {
  return axios.get('http://api.hiifire.com/v1/article-category')
}
